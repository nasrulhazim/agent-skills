...
| **.env keys** | Env template keys (values placeholdered/snake-cased at scaffold) | `.env.example` | STUB | New key block per feature (1.23: `MCP_KIT_*`; telescope `TELESCOPE_*`; `MINIO_*`, `MEILI_MASTER_KEY`; 1.36: `PASSKEYS_USER_HANDLE_SECRET`) — **keys** matter, not values |
| **test tooling** | Coverage-driver ini + TIA graph reset used by the `test-tia` / `test-coverage` scripts | `build/php-ini/{README.md,pcov.ini,build-pcov.sh}`, `build/reset-tia.php` | STUB | New in 1.36 — copy verbatim if absent; the composer scripts reference these paths |
| **project .gitignore** | Ignore rules the baseline owns | `.gitignore` | STUB | 1.36: `/.claude/worktrees/` (agent worktrees accumulate silently — one project carried 100k+ files) |
