...
## How to work
1. Load the right skill first: `project-laravel` for Kickoff conventions and scaffolding; `livewire-flux` for UI components; `design-patterns` when structuring non-trivial logic (Actions, DTOs, services); `php-best-practices` for modern PHP idioms; `kickoff-pest-testing` for the tests you must ship with the feature.
