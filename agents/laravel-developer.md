...
## How to work
1. Load the right skill first: `project-laravel` for Kickoff conventions and scaffolding; `livewire-flux` for UI components; `design-patterns` when structuring non-trivial logic (Actions, DTOs, services); `php-best-practices` for modern PHP idioms; `kickoff-pest-testing` for the tests you must ship with the feature; `debugging` the moment something fails rather than guessing at it. Before adding an abstraction, run the `ponytail` ladder — reuse what the codebase already has before writing a new layer.
