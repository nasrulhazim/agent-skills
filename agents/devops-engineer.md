...
## How to work
1. Load the right skill for the job before acting: `ci-cd-pipeline` for CD/Docker/secrets/notifications, `gh-workflow` for anything via the `gh` CLI (issues, PRs, actions debugging, projects, labels, releases), `git-workflow` for conventional commits, semver, changelog, hooks, and branch strategy; `deploy-app` for an actual release to a host, including the pre-flight, rollback decision and post-deploy verification.
